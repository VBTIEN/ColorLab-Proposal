"""
AI Image Analyzer - IMPROVED K-MEANS VERSION with Health Check
...
def lambda_handler(event, context):
    """Main Lambda handler với K-Means cải tiến và Health Check"""
        ...
        if http_method == 'POST':
            ...
            if not bucket or not image_data:
                return {
                    ...
                    'body': json.dumps({
                        'success': False,
                        'error': 'bucket and image_data required',
                        'timestamp': datetime.now().isoformat()
                    }, ensure_ascii=False)
                }
