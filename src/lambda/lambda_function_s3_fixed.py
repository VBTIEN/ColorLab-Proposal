"""
AI Image Analyzer - S3 Folder Fix Version
Sửa lỗi tạo folder và upload ảnh lên S3
"""
import json
import boto3
import base64
...
def lambda_handler(event, context):
    """Main Lambda handler với S3 folder fix"""
    ...
    try:
        ...
        # Process image with S3 folder fix
        result = process_image_with_s3_folder_fix(bucket, image_data, context)
...
def process_image_with_s3_folder_fix(bucket, image_data, context):
    """Xử lý ảnh với S3 folder creation fix"""
    
    try:
        # 1. Tạo folder structure và key
        folder_structure = create_s3_folder_structure()
        unique_id = str(uuid.uuid4())
        key = f"{folder_structure}/{unique_id}.jpg"
        
        print(f"📁 Tạo folder structure: {folder_structure}")
        ...
        print(f"❌ Lỗi trong process_image_with_s3_folder_fix: {str(e)}")
        raise
...
def create_s3_folder_structure():
    ...
    # Tạo folder structure: uploads/YYYY/MM/DD/HH
...
def create_fallback_analysis(rekognition_data):
    ...
    analysis = f"""
    📸 PHÂN TÍCH CHUYÊN NGHIỆP (Phiên bản 3.1 - S3 Folder Fix)
    ...
    🎨 ĐÁNH GIÁ TỔNG QUAN:
    • Ảnh đã được upload thành công với folder structure tự động
    • Hệ thống phân tích kỹ thuật hoạt động ổn định
    • Chất lượng dữ liệu phân tích: Tốt
    
    ✨ TÍNH NĂNG MỚI (v3.1):
    • ✅ S3 Folder Structure: Tự động tạo folder theo thời gian
